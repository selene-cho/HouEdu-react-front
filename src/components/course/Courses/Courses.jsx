...
import { fetchCourse, getCourses } from '../../../api/api';
...
export default function Courses() {
  ...
  const filteredCourses = data?.filter(
    (course) =>
      ...
      selectedCategory === course?.crs_category
  );
  ...
  return (
    <div className={styles.container}>
      <div className={styles.filterButtons}>
        ...
        <button
          type="button"
          className={selectedCategory === '기초' ? styles.active : ''}
          onClick={() => handleFilter('기초')}
        >
          기초
        </button>
        <button
          type="button"
          className={selectedCategory === '중급' ? styles.active : ''}
          onClick={() => handleFilter('중급')}
        >
          중급
        </button>
        <button
          type="button"
          className={selectedCategory === '고급' ? styles.active : ''}
          onClick={() => handleFilter('고급')}
        >
          고급
        </button>
      </div>
      ...
      {filteredCourses &&
        showCount < filteredCourses.length &&
        filteredCourses.length > SHOW_COUNT_DEFAULT && (
